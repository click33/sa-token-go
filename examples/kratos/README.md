# Kratos SaToken Example

这是一个基于当前 `SaToken` 项目的 `Kratos` 示例，结构和 `gf/gin` 示例保持一致，包含登录、登录态校验、角色校验、权限校验和注解式校验。
...
当前示例入口是 [main.go](/g:/code/go/my_project/sa-token-go/examples/kratos/main.go)。
